...
describe("swetrix_get_event_meta", () => {
  it("returns metadata keys and values", async () => {
    const r = await getTool(setup(), "swetrix_get_event_meta").handler({ pid: "p1", event: "signup", timeBucket: "day", period: "7d" });
...
describe("swetrix_get_property", () => {
  it("returns property aggregation", async () => {
    const r = await getTool(setup(), "swetrix_get_property").handler({ pid: "p1", property: "author", timeBucket: "day", period: "7d" });
...
describe("swetrix_get_custom_events", () => {
  it("returns empty array on no events", async () => {
    const r = await getTool(setup(), "swetrix_get_custom_events").handler({ pid: "p1", customEvents: '["signup"]', timeBucket: "day", period: "7d" });
